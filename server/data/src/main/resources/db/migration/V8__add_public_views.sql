...
SELECT
    ...
    video.updated_at
FROM private.video
         LEFT JOIN private.video_url ON video.id = video_url.video_id
         LEFT JOIN private.video_label ON video.id = video_label.video_id
         LEFT JOIN private.label ON video_label.label_id = label.id
WHERE visible
GROUP BY video.id;
