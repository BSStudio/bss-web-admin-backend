...
import { createTestApiClient, createTestLabel } from '../utils/index.js';
...
describe('Label API Integration Tests', () => {
  ...
  describe('Label CRUD Operations', () => {
    ...
    it('should retrieve all labels', async () => {
      ...
      const testLabels = labels.filter((label) =>
        label.name.includes('TEST_INTEGRATION'),
      );
    ...
    it('should create multiple labels with unique names', async () => {
      const label1 = createTestLabel({
        name: 'Label 1 TEST_INTEGRATION_UNIQUE_1',
      });
      const label2 = createTestLabel({
        name: 'Label 2 TEST_INTEGRATION_UNIQUE_2',
      });
