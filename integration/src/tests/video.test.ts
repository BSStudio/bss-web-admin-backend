...
import type { BssApiClient, Video, DetailedVideo } from '../client/index.js';
import { createTestVideo, createTestVideoUpdate, createTestApiClient } from '../utils/index.js';
...
describe('Video API Integration Tests', () => {
  ...
  describe('Video CRUD Operations', () => {
    ...
    it('should retrieve all videos', async () => {
      ...
      const testVideos = videos.filter(video => video.title.includes('TEST_INTEGRATION'));
    ...
    it('should retrieve a specific video by ID', async () => {
      ...
      const retrievedVideo = await apiClient.videos.getVideoById(createdVideo.id);
    ...
    it('should update an existing video', async () => {
      ...
      const updatedVideo = await apiClient.videos.updateVideo(createdVideo.id, updateVideo);
    ...
    it('should change video visibility', async () => {
      ...
      const updatedIds = await apiClient.videos.changeVideoVisibility([createdVideo.id], false);
    ...
    it('should delete a video', async () => {
      ...
      await expect(apiClient.videos.deleteVideo(createdVideo.id)).resolves.toBeUndefined();
      
      // Verify it's gone
      await expect(apiClient.videos.getVideoById(createdVideo.id)).rejects.toThrow();
    });
  });
  ...
  describe('Video Validation', () => {
    ...
    it('should handle bulk visibility change with mixed valid/invalid IDs', async () => {
      ...
      try {
        const result = await apiClient.videos.changeVideoVisibility([createdVideo.id, invalidId], true);
