...
export async function setup(project: TestProject) {
  ...
  const environment = new DockerComposeEnvironment('./../', [
    ...
    .withLogConsumer('app', (stream) => {
      console.log(`[APP] ${stream.getData()}`);
    })
    .withLogConsumer('postgres', (stream) => {
      console.log(`[DB] ${stream.getData()}`);
    });
  ...
  try {
    ...
    if (
      cleanupSummary.events +
        cleanupSummary.videos +
        cleanupSummary.members +
        cleanupSummary.labels >
      0
    ) {
    ...
    console.warn(
      '⚠️  Could not clean up leftover test data (this is normal if the database is empty)',
    );
  }
}
