import { DbUtils, videoEntity } from '../../database'
import { createVideo, CreateVideo } from '../../endpoints/app'
import { UUID_REGEX, dateToday } from '../../util'
import { mockCreateVideoFolder, resetMocks, verifyCreateVideoFolder } from '../../endpoints/file-api/file.endpoint'

describe('post /api/v1/video', () => {
  const dbUtils = new DbUtils()
  beforeEach(async () => Promise.all([resetMocks(), dbUtils.beforeEach()]))
  afterAll(async () => Promise.all([resetMocks(), dbUtils.afterAll()]))

  const id = '01234567-0123-0123-0123-0123456789ab'
  const url = 'url'
  const title = 'title'

  it('should return ok and created video', async () => {
    expect.assertions(4)
    await mockCreateVideoFolder()

    const createVideoBody: CreateVideo = { url, title }
    const response = await createVideo(createVideoBody)
    const mockCalls = await verifyCreateVideoFolder()

    expect(mockCalls).toBe(1)
    expect(response.status).toBe(201)
    expect(response.headers.location).toBe(`${globalThis.baseUrl.app}/api/v1/video/${response.data.id}`)
    expect(response.data).toMatchObject({
      id: expect.stringMatching(UUID_REGEX) as string,
      url,
      title,
      visible: false,
      uploadedAt: dateToday(),
    })
  })
  it('should return internal server error when url already exist', async () => {
    expect.assertions(2)
    await dbUtils.addVideos([videoEntity({ id, url, title: 'otherTitle' })])
    const createVideoBody: CreateVideo = { url, title }

    const response = await createVideo(createVideoBody)
    const mockCalls = await verifyCreateVideoFolder()

    expect(mockCalls).toBe(0)
    expect(response.status).toBe(500)
  })
  it('should return internal server error when title already exist', async () => {
    expect.assertions(2)
    await dbUtils.addVideos([videoEntity({ id, url: 'otherUrl', title })])
    const createVideoBody: CreateVideo = { url, title }

    const response = await createVideo(createVideoBody)
    const mockCalls = await verifyCreateVideoFolder()

    expect(mockCalls).toBe(0)
    expect(response.status).toBe(500)
  })
})
